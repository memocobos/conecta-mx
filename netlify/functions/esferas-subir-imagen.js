// =============================================================================
// esferas-subir-imagen
//
// Sube una imagen del FESTIVAL de una Esfera (portada o lineup) al bucket PÚBLICO
// 'mapas-eventos' (proyecto KH npgnhsmwpcipxgvfxrho) y devuelve la URL pública.
// El caller (kamehouse) guarda esa URL dentro del objeto `festival`.
//
// Calcado de esferas-subir-mapa. Difiere en: valida `tipo` ∈ {portada,lineup} y
// el path es `<slug>-<tipo>.<ext>` (NO pisa el mapa, que es `<slug>.<ext>`).
//
// Body JSON: { slug, dataUrl, tipo }  — dataUrl = 'data:image/<sub>;base64,...'
// Respuesta: { ok:true, url:'<público>', path:'<slug>-<tipo>.<ext>' }
//
...
const TIPOS = ['portada', 'lineup'];

// Acepta data URL `data:image/<sub>;base64,XYZ`. Devuelve {bytes, mime, ext} o
// {error}/{tooBig}. Solo webp/jpg/png. Idéntico a esferas-subir-mapa.
...
exports.handler = async (event) => {
  ...
  const tipo = (body && typeof body.tipo === 'string') ? body.tipo.trim().toLowerCase() : '';
  if (!TIPOS.includes(tipo)) return { statusCode: 400, headers, body: JSON.stringify({ error: "tipo inválido (portada|lineup)" }) };
  ...
  const path = `${slug}-${tipo}.${parsed.ext}`;
  try {
